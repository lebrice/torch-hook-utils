...
def save_maxpool_indices(
    module_or_modules: nn.Module | Mapping[str, _MaxPoolLayer]
...
def use_indices_in_maxunpool(
    module_or_modules: nn.Module | Mapping[str, nn.modules.pooling._MaxUnpoolNd],
    maxpool_indices: dict[str, Tensor],
    ...
    if not consume_indices:
        maxpool_indices = maxpool_indices.copy()
    ...
    if isinstance(module_or_modules, nn.Module):
        ...
        named_maxunpool_modules = module_or_modules

    for module_name, module in named_maxunpool_modules.items():
        if module_name not in maxpool_indices:
        ...
        indices = maxpool_indices.pop(module_name)
